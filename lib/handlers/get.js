  ...
  Promise.all(tasks)
    .then(results => {
      const queryWhere = results.reduce((out, currentWhere) => {
        if (currentWhere.concatIDs) {
          if (!out[currentWhere.concatIDs]) out[currentWhere.concatIDs] = []
          
          out[currentWhere.concatIDs] = out[currentWhere.concatIDs].concat(currentWhere.ids)
        }
        else          
