...
class Multicolour_Route_Templates {
   ...
   */
  create_related_in_payload(request, model) {
    const create_or_find_item = (item, target_model) => {
      if (item && item.toString() === "[object Object]")
        ...
        return target_model.find({id: item})
      else
    ...
    this.debug("Finding/creating related attributes for", related_models_names)
    ...
    return Promise.all(related_models_names.map(model_name => {
      ...
      // Get the payload.
      const payload = request.payload[model_name]

      // Do the work.
      if (Array.isArray(payload))
        return Promise.all(payload.map(item => create_or_find_item(item, target_model)))

      return create_or_find_item(payload, target_model)
        .then(result => ({
          key: model_name,
  ...
  POST(model, request, callback) {
    ...
    this.create_related_in_payload(request, model)
      .then(results => {
        const related = {}
  ...
  GET(model, request, callback, from_post) {
    ...
    const per_page = qs.perPage || this.multicolour.get("config").get("settings").results.per_page
    delete qs.perPage
    ...
    if (this.collection_has_associations(model)) {
      ...
      // Otherwise, populate what's been asked for.
        ...
        Promise.all(tasks)
          ...
          .then(() => {
            query.populateAll().exec(exec_callback)
          })
          .catch(err => callback(err, null, model))
      }
    }
  ...
  PUT(model, request, callback) {
    ...
    model.findOne(qs, (err, models) => {
      ...
      if (!models) {
        ...
        return this.create_related_in_payload(request, model)
          .then(results => {
            const related = {}
