...
class Multicolour_Waterline_Generator extends Map {
  ...
  generate_model(blueprint) {
    ...
    blueprint.model.connection = blueprint.model.connection || this.request("host").get("env") || "development"
  ...
  start() {
    ...
    return new Promise((resolve, reject) => {
      this.get("waterline").initialize(updatedConfig, (err, ontology) => {
        ...
        this.debug(Object.keys(ontology.datastores.development))
        // These models are unrelated to one another, first pass.
        this.raw_waterline_joi_models  = Object.keys(ontology.collections)
          .map(name => ontology.collections[name])
          .filter(model => !model.meta.junctionTable)
          .map(model => ({
            schema: waterline_joi(model.attributes, false),
            identity: model._adapter.identity,
          }))
        ...
        this.raw_waterline_joi_models_related = Object.keys(ontology.collections)
          .map(name => ontology.collections[name])
          .filter(model => !model.meta.junctionTable)
          .map(model => {
            const related_columns = get_related_columns(model)
            const validation_model = Object.assign({}, this.raw_waterline_joi_models[model._adapter.identity])
        ...
        const fixes_for = ontology.datastores.development.adapter.identity
        this.fix_waterline(fixes_for, ontology.datastores)
          .then(() => {
