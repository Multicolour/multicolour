...
class CLI_Init {
  ...
  program() {
    ...
    this.CLI.program
      ...
      .option("-t, --target <target>", "Where to init.", "./")
      .option("-d, --install-dependencies <boolean>", "Have a cache or no/slow internet? Set this to false", true)
   ...
   * @param {Command} command arguments object.
   * @return {Promise} promise in resolved state.
   */
  run(target, command) {
    this.welcome_message()

    // Do we want to install dependencies?
    this.run_npm_install = Boolean(command.installDependencies)
    ...
    return inquirer.prompt([
      ...
      {
        when: answers => answers.database !== "memory" && answers.database !== "sqlite3" && answers.database !== "nedb",
        type: "input",
        name: "database_host",
      ...
      {
        when: answers => answers.database !== "memory" && answers.database !== "nedb",
        type: "input",
  ...
  deal_with_init_answers(answers) {
    ...
    const wantsJWT = Boolean(answers.authentication.find(answer => answer === "Email/Password (JWT)"))
    ...
    if (wantsJWT || wantsOauth) {
      console.log("Generating a secure master key") // eslint-disable-line
  ...
  ask_for_default_user(answers) {
    /* eslint-disable */
    console.info(chalk.bold.underline.yellow("Default user details."))
  ...
  create_database(answers) {
    ...
    return new Promise((resolve, reject) => {
      ...
      context.initialize({
        ...
        // MySQL requires backticks and postgres double quotes.
        const create_statement = answers.database === "postgresql"
          ? `CREATE DATABASE "${answers.project_name}";`
          : `CREATE DATABASE \`${answers.project_name}\`;`

        // Create the database.
        ontology.collections.check.query(create_statement, err => {
          // Check for errors and check the error message.
          if (
            err
            && err.message !== `database "${answers.project_name}" already exists`
  ...
  run_all_tasks(answers) {
    Promise.all([
      ...
      this.run_npm_install ? this.install_dependencies() : Promise.resolve(),
    ])
      .then(() => {
  ...
  install_dependencies() {
    ...
    const spinner = new Spinner("Installing your API's dependencies- Not any databases. %s")
  ...
  get_oauth_config(answers) {
    ...
    /* eslint-disable */
    console.info(chalk.italic.blue("If you want to use environmentals use them like so $NODE_ENV or $TWITTER_SECRET"))
    /* eslint-enable */
      ...
      .then(provider_answers => {
        answers.auth.providers = providers.map(strategy => {
          return {
            ...
            clientId: replace_env_values(provider_answers[strategy + "_id"]),
            clientSecret: replace_env_values(provider_answers[strategy + "_secret"]),
            isSecure: false
          }
        })
        ...
        if (err)
          reject({
            ...
            error: {
              ...
              data: JSON.stringify(user_config, null, 2)
            }
          })
        else {
  ...
  write_package_json_file(answers) {
    ...
    if (answers.authentication.length > 0) {
      if (answers.authentication.find(type => type === "Email/Password (JWT)")) {
        user_package.dependencies["multicolour-hapi-jwt"] = "^0.2.0"
        server_plugin.plugins.push("multicolour-hapi-jwt")
      }

      if (answers.authentication.filter(type => type !== "Email/Password (JWT)").length > 0) {
        user_package.dependencies["multicolour-hapi-oauth"] = "^1.0.0"
        server_plugin.plugins.push("multicolour-hapi-oauth")
      }
