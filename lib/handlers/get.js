 ...
 */
function getJunctionTableFromModelAndRelatedColumn(model, attribute) {
  // tag_tags_tag__variant_tags
  const junctionTableName = Object.keys(model.waterline.schema)
    .find(schemaName => {
      ...
      const junctionName = `${tables[1]}_${attribute}_${tables[1]}__${tables[0]}_${attribute}`

      return schemaName === junctionName
    })
...
module.exports = function GETHandler(model, request, callback, from_post) {
  ...
  const tasks = Object.keys(populate_where).map(relationship => {
    ...
    if (column.collection && junctionTable)
      return findRelatedCollectionIDsByCriteria(collections, model.adapter.identity, target.adapter.identity, relationship, junctionTable, populate_where[relationship])
        .then(results => ({
