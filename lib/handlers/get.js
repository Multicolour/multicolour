...
function findRelatedCollectionIDsByCriteria(models, junctionTable, subQueryCriteria) {
  ...
  const primaryTable = models[junctionTable.tables[0]]
...
module.exports = function GETHandler(model, request, callback, from_post) {
  ...
  const tasks = Object.keys(populate_where).map(relationship => {
    ...
    const junctionTable = getJunctionTableFromModelAndRelatedColumn(target, model.adapter.identity, target.adapter.identity)
    ...
    if (column.model)
      return findRelatedModelIDByCriteria(model, populate_where[relationship], column.via)
