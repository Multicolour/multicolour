...
class Multicolour_Default_Validation {
  ...
  generate_schemas(verb) {
    ...
    Object.keys(collections).forEach(collection_name => {
      if (verb === "post")
        delete collections[collection_name].attributes.id
      ...
      .forEach(collection_name => {
        ...
        Object.keys(attributes)
          ...
          .forEach(attribute => {
            ...
            set_to = Joi.object(set_to)

            // Add the relation.
            if (attributes[attribute].model)
              collection[attribute] = Joi.alternatives().try(set_to, Joi.number(), Joi.string()).allow(null)
