...
const fix_postgres_sequences = collections => Promise.all(Object.keys(collections)
  ...
  .map(collection => new Promise(resolve => {
    const seq_name = (`${collection.adapter.identity}_id_seq`).substring(59)
    const query = `SELECT pg_catalog.setval('${seq_name}', max(id), true) FROM "${collection.adapter.identity}";`
