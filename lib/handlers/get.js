...
function getJunctionTableNameFromModelAndRelatedColumn(model, parentTableName, relatedColumnTableName) {
...
module.exports = function GETHandler(model, request, callback, from_post) {
  ...
  const tasks = Object.keys(populate_where).map(relationship => {
    ...
    getJunctionTableNameFromModelAndRelatedColumn(target, model.adapter.identity, target.adapter.identity)
